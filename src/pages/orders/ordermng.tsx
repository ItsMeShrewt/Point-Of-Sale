...
const Orders: React.FC = () => {
  ...
  const [orders, setOrders] = useState<{ name: string; description: string; price: number }[]>([]);
  ...
  useEffect(() => {
    if (gridRef.current) {
      ...
      const handleClick = (event: Event) => {
        ...
        const productDescription = button.getAttribute("data-description") || "";  // added
        const productPrice = parseFloat(button.getAttribute("data-price") || "0");
      
        setOrders((prevOrders) => {
          const exists = prevOrders.some(
            (order) => order.name === productName && order.description === productDescription // updated condition
          );
          if (!exists) {
            return [...prevOrders, { name: productName, description: productDescription, price: productPrice }];
          }
          return prevOrders;
        });
      };
    ...
    <>
      ...
      <div className="main-content app-content">
        <div className="container-fluid">
          <Breadcrumb title="Manage Orders" links={[{ text: " Dashboard", link: "/product-sales" }]} active="Sales Orders"                buttons={
                  <Link to="" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded flex items-center gap-2">
            ...
            <div className="xxl:col-span-4 col-span-12">
              <div className="box overflow-hidden main-content-card flex flex-col justify-between" style={{ height: "815px" }}>
                ...
                <div className="p-5 border-t bg-gray-50 flex flex-col items-stretch gap-3">
                  ...
                  <TransactionButtons 
                    onProcess={() => { setOrders([]); setAmountGiven(0); setDeliveryFee(0); }}
